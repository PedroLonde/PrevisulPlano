...
@st.cache
def load_data(novo_file, antigo_file):
    nov = pd.read_excel(novo_file, header=1)
    ant = pd.read_excel(antigo_file)
    return nov, ant

# Interface do Streamlit para upload de arquivos
st.title("Relatório de Beneficiários")

novo_file = st.file_uploader("Carregar Relatório Novo", type=["xlsx"])
antigo_file = st.file_uploader("Carregar Relatório Antigo", type=["xlsx"])

if novo_file and antigo_file:
    # Carregar os dados
    nov, ant = load_data(novo_file, antigo_file)
